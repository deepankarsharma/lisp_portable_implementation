...
#define INTERNAL	8	/* interal object -- not in symbol table */
...
#define CHECKsym2(yyy,xxx)				\
	if (xxx->flag >= VOID || xxx->flag == CONSTANT) error(yyy,err_sym2,xxx)
...
extern kerncell Lconvset(), Ldconvset(), Lconvlist(), Ldconvlist(),
		Veset(), Uiset(), iset(), unionaux(), remrep(),
