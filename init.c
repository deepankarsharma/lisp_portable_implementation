...
initialize ()
{
	...
	_tempstr = mkstr("");
	/* constants: */
	nil	   = newsym("nil",    CONSTANT,NULL); nil->bind = nil->prop = NIL;
	ttt	   = newsym("t",      CONSTANT,NULL); ttt->bind = TTT;
	eofsym	   = newsym("eof",    CONSTANT,NULL); eofsym->bind = CONVcell(eofsym);
	...
	sumsym	   = newsym("sum",     VBINARY,Vsum);
	prodsym	   = newsym("prod",    VBINARY,Vprod);
